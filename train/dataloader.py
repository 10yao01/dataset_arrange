...
class PASTIS_Dataset(tdata.Dataset):
    def __init__(
        ...
        Args:
            ...
            norm (bool): If true, images are standardised using pre-computed
                ...
                (在文件中包含NORM_S2_patch.json文件中包含图像的means和stds)
        ...
        for s in sats:
            ...
            # patch_id    all date of this patch
            ...
            date_table = date_table.fillna(0)  # 用0做数据填充
            self.date_tables[s] = {
        ...
        # # 将patch_id 写入json文件
        # print("type_id_patches{}".format(type(self.id_patches)))
        # # 遍历
        # patch_dict = {}
        # for i in self.id_patches:
        #     patch_dict[i] = {"date_id": []}
        # with open("data_collect.json", "w") as f:
        #     json.dump(patch_dict, f, indent=4, ensure_ascii=False)
        ...
        # 选取使用的时相
        self.data_select_file = data_select_file
        with open(self.data_select_file, "r") as load_f:
            self.load_dict = json.load(load_f)
        self.load_dict = self.load_dict["features"]
    ...
    def __getitem__(self, item):
        ...
        # target_date_id = self.load_dict["{}".format(id_patch)]["date_id"]
        target_date_id = []
        for i in self.load_dict:
            if i["ID_PATCH"] == "{}".format(id_patch):
                target_date_id = list(i["DATE"].keys())
        target_date_id = list(map(int, target_date_id[:]))
        if self.mem16:
            ...
            # 目前选择的波段为RGB波段
            # data只有一个s2的key
            # 选择时间（通过选择后的json文件进行下标匹配选择）
            return (
                (
                    data["S2"][:, [0, 1, 2], :, :][target_date_id, :, :, :],
                    dates["S2"][target_date_id],
                ),
                target,
                # 仅仅是为了筛选图片
                np.array([id_patch]),
